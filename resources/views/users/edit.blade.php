                        ...
                        <form method="post" action="{{ route('users.update', $user) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('employeeid') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="employeeid" id="input-employeeid" class="form-control form-control-alternative{{ $errors->has('employeeid') ? ' is-invalid' : '' }}" placeholder="{{ __('Employee ID') }}" value="{{ old('employeeid', $user->employeeid) }}"" disabled>
                                    @include('alerts.feedback', ['field' => 'employeeid'])
                                </div>
                                <div class="form-group{{ $errors->has('username') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-username">{{ __('Username') }}</label>
                                    <input type="text" name="username" id="input-username" class="form-control form-control-alternative{{ $errors->has('username') ? ' is-invalid' : '' }}" placeholder="{{ __('Username') }}" value="{{ old('username', $user->username) }}">
